...
export default function GameDashboard({ players }: GameDashboardProps) {
  ...
  const [gameScores, setGameScores] = useState<[GameScores, GameScores, GameScores]>([
    { player1: [], player2: [] },
    { player1: [], player2: [] },
    { player1: [], player2: [] },
  ])
  ...
  const [gamesCompleted, setGamesCompleted] = useState<[boolean, boolean, boolean]>([false, false, false])
  ...
  const updateGameScores = (winner: number | null, player1Score: number, player2Score: number) => {
    if (winner === null) return // No updates for draws

    const newGameScores = [...gameScores] as [GameScores, GameScores, GameScores]
    ...
    for (let i = 0; i < 3; i++) {
      if (!newGamesCompleted[i]) {
        const player1Total = newGameScores[i].player1.reduce((sum, score) => sum + score, 0)
        const player2Total = newGameScores[i].player2.reduce((sum, score) => sum + score, 0)
  ...
  const totalScores = getTotalScores()
  const overallWinner = completedGamesWon[0] >= 2 ? players[0] : completedGamesWon[1] >= 2 ? players[1] : null

  return (
    <div className="space-y-6">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <ScoreInput players={players} onAddHand={addHand} disabled={overallWinner !== null} />
